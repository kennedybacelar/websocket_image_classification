...
def train():
    ...
    data_directory = global_config.get("train", {}).get("resources", {}).get("train_dataset", "train_dataset")
    ...
    validation_split = global_config.get("train", {}).get("params", {}).get("validation_split", 0.4)
    ...
    model = build_model(input_shape=(224, 224, 3), num_classes=train_dataset.num_classes)
...
def save_model(model):
    filename = global_config.get("models", {}).get("paths", {}).get("model", "models/heroes.h5")
    model.save(filename)


def save_class_indices_as_json(class_indexes: dict):
    # Save the class indices mapping as a JSON file
    filename = global_config.get("models", {}).get("paths", {}).get("indices", "models/heroes_class_indices.json")
